...
A Spring Boot-based caching proxy application that intercepts client requests, caches external API responses using Spring Cache with Caffeine, and reduces latency and redundant API calls.
...
To use the proxy, send a GET request to the `/proxy` endpoint with the `url` query parameter pointing to the external API you want to fetch.
